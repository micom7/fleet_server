...
- Pull через Outbound API авто: `/data`, `/alarms`, `/channels`
- Записує в центральну БД batch-інсертом
...
| `vehicles` | Авто: VPN IP, порт, last_seen_at, sync_status |
...
- Сервер кожні 2 сек робить `GET /data/latest` до конкретного авто (Outbound API)
...
`/data/latest` — новий легкий endpoint в Outbound API на авто:
```sql
...
- **Авто** — поточні значення (SSE/WS), вбудована Grafana (iframe), активні тривоги, кнопка "Live"
...
## Що потрібно додати в Outbound API на авто

```
GET /data?from=<iso>&to=<iso>      → вимірювання за діапазон
GET /data/latest                   → останнє значення по кожному каналу
GET /alarms?from=<iso>&to=<iso>    → тривоги за діапазон
GET /channels                      → конфігурація каналів
```
...
│   ├── routes/
│   └── ws_live.py
├── db/              # Схема центральної БД
│   └── 01_init.sql
├── grafana/         # Provisioning, auth proxy config
├── web/             # Web UI (шаблони або SPA)
├── docker-compose.yml
└── .env
```
...
| 3 | Outbound API на авто | `/data`, `/data/latest`, `/alarms`, `/channels` |
| 4 | Sync Service | Pull-сервіс, sync_journal, статуси авто |
